    ...
    <div class="card shadow border-0 rounded-4" style="width: 100%; max-width: 760px;">
      <div class="card-body p-4 p-md-5">
        <div class="text-center mb-4">
          ...
          <p class="text-muted mb-0">Preencha os dados de envio de e-mail para concluir o setup da API.</p>
        </div>

        <div v-if="bootstrapErro" class="alert alert-warning">
          {{ bootstrapErro }}
        </div>
        ...
        <form @submit.prevent="salvar">
          ...
          <div class="d-grid">
            <button type="submit" class="btn btn-primary" :disabled="carregando || !bootstrapOk">
              {{ carregando ? 'Salvando...' : 'Concluir configuração' }}
            </button>
          </div>
        </form>
      </div>
